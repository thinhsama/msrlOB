...
class DQN():
    ...
    def learn(self):
        ...
        # Double DQN - Select action using eval_net, evaluate using target_net
        next_state_action = self.eval_net(batch_next_state).max(1)[1].view(BATCH_SIZE, 1).to(self.device)
        q_next = self.target_net(batch_next_state).gather(1, next_state_action).detach()
        
        q_target_unterminated = batch_reward + GAMMA * q_next
